...
public class TerminalApiClientWagerTriTest {
    ...
    public void C_givenGoodResponse_whenWagerRequest_thenResponseObjectCreated() {
        ...
        for (int i = 0; i < tests; i++) {
            ...
            wager = new WagerBuilder().buildTriTest();
            ...
            if (response.getStatus() == Response.Status.OK.getStatusCode()) {
                ...
                if (wagerResponse.getTicket()!=null){
                    if (wagerResponse.getTicket().getTsn()!=null&&wagerResponse.getTicket().getTsn().length()>0){
                        System.out.println(i+"("+elapsedTime + " ms) tri bet. " + wager + " response : " + wagerResponse);
                    }else {
                        System.err.println(i + "(" + elapsedTime + " ms) tri bet. NO TSN. " + wager + " response : " + wagerResponse);
                    }
                }else{
                    System.err.println(i+"("+elapsedTime + " ms) tri bet. NO Ticket details. " + wager + " response : " + wagerResponse);
                }
            } else {
                System.err.println(i+"("+elapsedTime + " ms) tri bet. " + wager + " response : " + response.readEntity(String.class));
            }
