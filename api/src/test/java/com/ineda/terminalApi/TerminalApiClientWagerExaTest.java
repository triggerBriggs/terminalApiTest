...
public class TerminalApiClientWagerExaTest {
    ...
    public void C_givenGoodResponse_whenWagerRequest_thenResponseObjectCreated() {
        ...
        for (int i = 0; i < tests; i++) {
            ...
            wager = new WagerBuilder().buildExaTest();
            ...
            if (response.getStatus() == Response.Status.OK.getStatusCode()) {
                ...
                if (wagerResponse.getTicket()!=null){
                    if (wagerResponse.getTicket().getTsn()!=null&&wagerResponse.getTicket().getTsn().length()>0){
                        System.out.println(i+"("+elapsedTime + " ms) exe bet. " + wager + " response : " + wagerResponse);
                    }else {
                        System.err.println(i + "(" + elapsedTime + " ms) exe bet. NO TSN. " + wager + " response : " + wagerResponse);
                    }
                }else{
                    System.err.println(i+"("+elapsedTime + " ms) exe bet. NO Ticket details. " + wager + " response : " + wagerResponse);
                }
            } else {
                System.err.println(i+"("+elapsedTime + " ms) exe bet. " + wager + " response : " + response.readEntity(String.class));
            }
