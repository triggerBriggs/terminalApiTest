package com.ineda.terminalApi.wagering;
...
public class TerminalApiWagerProvider extends ManagedServiceImpl implements
        WagerProvider, ErrorCodes, ServiceCodes {

    private static final String API_VERSION = "v2";

    private TerminalApiClient client;
    private SessionFactory sf;

    private static final Logger log = Logger
            .getLogger(TerminalApiWagerProvider.class);
    ...
    public void init() throws ServiceException {
        client = new TerminalApiClient(endPoint, API_VERSION,
                "7414",
                "password",
                "643cd362ff15427c",
                jerseyLogPath);
        log.debug("Terminal API client created and ready for wagering !!");
    }
    ...
    public CancelResult cancelTicket(Ticket ticket, Pool pool) throws UnimplementedException, InvalidCancelException, WagerProviderException, ToteUnavailableException {
        ...
        try {
            if (cr.isGood()) {
                ...
                if (cr.getStatus() != TerminalApiError.ALREADY_CANCELLED.getCode()) {
